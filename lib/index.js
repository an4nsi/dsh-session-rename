...
const KNOWN_TAGS = 'dsh / zmk / 計劃 / nixos / ha / brain / bifrost / git / pi / pasta / astrbot / backup / 購物 / 打印机'

export function apply(ctx) {
  ctx.tools.register(defineTool({
    name: 'rename_session',
    description: 'Finalize the title of the CURRENT session. Use it when the session wraps up (right after the brain note is written), not at the first message. Title format: "[tag] [tag] one-line summary" - known tags: ' + KNOWN_TAGS + '; new tags are allowed. Max 80 UTF-8 bytes; the title is pinned until renamed again.',
    parameters: {
      title: {
        type: 'string',
        required: true,
